...
last_time_step = df_preds['time_step'].iloc[-1]
last_date = df_preds.index[-1]
...
chart_df = concat([df_preds, df_future])
chart_df
```

:::{.callout-note}
The population and error values for future dates are null, because we don't know them yet. Although we are able to make predictions about these values.
:::
