...
Then the secret values are accessed within the notebook using the `userdata` dictionary from the `google.colab` sub-module. In the example below, we are storing the value of the notebook secret called `SECRET_MESSAGE` in a Python variable called `message`, and then doing something with that value (in this case printing it):
