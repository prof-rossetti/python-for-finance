...
Control flow structures in Python include [conditionals](onditional-logic.qmd) (\"if\" statements), [custom functions](custom-functions.qmd), [error handling](errors.qmd), and loops (\"for" loops vs [\"while\" loops](while-loops.qmd)). We will study most of these structures in this chapter, then we will return later to cover \"for\" loops after studying the `list` datatype in more detail.
