...
Here is an example of each of the most popular datatypes. We will cover each of these datatypes and their operations in more detail, but for now just know what these datatypes look like:
...
Container datatypes that hold many values:

| Datatype | Description  |  Example |
|---|---|---|
| List  | A sequence of values. Order matters. Can contain duplicates. Mutable (can be updated and changed). | `["vanilla", "chocolate", "strawberry"]`|
| Tuple  | A sequence of values. Immutable (cannot be changed). | `("vanilla", "chocolate", "strawberry")`|
| Set  | Like a list, but without order, and cannot contain duplicates. | `{"vanilla", "chocolate", "strawberry"}`|
...
In this case, we tried to use the plus sign operator with mixed datatypes. Python threw an error because it doesn't know what operation to perform. Should it perform addition? Or concatenation? This is why datatypes are so important to pay attention to!
...
![A shelf of Polos with varying colors and sizes](./../../../images/polos.png)
...
Whereas in **object oriented programming**, the objects are the stars of the show, and we invoke methods and properties directly on them.
...
**Properties** are attributes or characteristics of the object. For example, all polos will have a "size", a "color" and "price", even though their individual values may vary. In other words, we can have a yellow polo, and a blue polo, but they each have a "color".
...
**Methods** are behaviors that an object can have. A method is like a function in the sense that it represents the performance of some verb or action.
